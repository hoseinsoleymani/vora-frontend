...
export async function fetchPriceCalendarData(origin: string, destination: string, departureDate: string) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Failed to fetch price calendar data: ${res.status} ${errorText}`);
    }

    const data = await res.json();

    if (typeof data !== 'object' || data === null) {
      console.error('Server Action: Invalid data format received, expected object');
      return [];
    }
...
function processFlightData(flight: FlightApiResponse): Flight { 
  ...
  if (stopCount > 0) {
    stopInfo = segments.slice(0, -1).map((currentSegment, i) => {
      ...
      const layoverDuration = Math.floor((arrivalTime.getTime() - departureTime.getTime()) / (1000 * 60));
...
export async function fetchFlightData(
  ...
  try {
    ...
    if (!res.ok) {
      const errorText = await res.text();
      throw new Error(`Failed to fetch flight data: ${res.status} ${errorText}`);
    }

    const data = await res.json();

    if (!data.results || !Array.isArray(data.results)) {
      console.error('Server Action: Invalid response format, missing results array');
      return {
        results: [],
        totalPages: 0
      };
